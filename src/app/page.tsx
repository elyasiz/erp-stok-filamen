...
import { BarChart3, Bell, Boxes, ChevronDown, ClipboardCheck, ClipboardPenLine, Clock3, History, LayoutDashboard, Menu, PackagePlus, Plus, Scale, ScanBarcode, ScanLine, Search, Settings, Users, X } from "lucide-react";
...
const navigation: Array<{ label: string; items: Array<{ label: string; icon: LucideIcon; view: ViewId; count?: string }> }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Verifikasi gram", icon: Scale, view: "weighings" },
...
function Sidebar({ open, activeView, onClose, onSelect, reports }: { reports: ReportState; open: boolean; activeView: ViewId; onClose: () => void; onSelect: (view: ViewId) => void }) {
  ...
  const allowed = (view: ViewId) => user && (view === "users" || view === "settings" ? user.role === "OWNER" : ["reports", "history", "receipt", "activity", "corrections", "weighings"].includes(view) ? isStaff(user) : true);
...
function Workspace() {
  ...
  const selectView = (nextView: ViewId, sessionId?: string) => {
    ...
    if (["receipt", "reports", "history", "activity", "corrections", "weighings"].includes(nextView) && !isStaff(user)) return;
    setView(nextView);
