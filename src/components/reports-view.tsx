...
import { Download, History, PackagePlus, ScanBarcode, Weight } from "lucide-react";
import { useState } from "react";
import type { ReactNode } from "react";
import { filterReportPeriod, resultLabels, sessionStatusLabels, stockStatusLabels, toCsv, usageTotals } from "@/lib/report-data";
...
export default function ReportsView({ state, ledgerOnly = false }: { state: ReportState; ledgerOnly?: boolean }) {
  ...
  const cards = [
    ...
    { kind: "usages" as const, title: "Penggunaan", stat: `${totals.count} sesi selesai · ${formatKg(totals.grams)}`, icon: ScanBarcode, description: `${period.usages.filter((item) => item.status === "ACTIVE").length} sesi aktif dalam periode` },
  ...
  const downloadCsv = (report: ReportKind) => {
    ...
    anchor.href = url;
    anchor.download = `${report}-tidigo-${report === "inventory" ? data.today : month || "semua-periode"}.csv`;
  ...
  return <>
    <div className="module-heading"><div><span className="eyebrow">{ledgerOnly ? "INVENTORY · RIWAYAT" : "PELAPORAN"}</span><h1>{ledgerOnly ? "Riwayat pergerakan stok" : "Laporan"}</h1><p>Data stok, penerimaan, dan penggunaan yang tersimpan.</p></div><div className="heading-actions"><ReportRefresh state={state} /><button className="button secondary" onClick={() => downloadCsv(kind)}><Download size={17} /> Ekspor {title}</button></div></div>
    <div className="report-period"><label>Periode transaksi <input type="month" value={month} onChange={(event) => setMonth(event.target.value)} aria-label="Bulan laporan" /></label><button className="button secondary" onClick={() => setMonth(data.currentMonth)}>Bulan ini</button><button className="button secondary" onClick={() => setMonth("")}>Semua periode</button><small>{month || "Semua periode"} · Stok selalu menunjukkan kondisi terkini.</small></div>
    {!ledgerOnly ? <section className="report-grid">{cards.map(({ icon: Icon, ...card }) => <article className={`report-card ${kind === card.kind ? "selected" : ""}`} key={card.kind}><span className="report-icon"><Icon size={21} /></span><div><h2>{card.title}</h2><strong>{card.stat}</strong><p>{card.description}</p></div><button aria-pressed={kind === card.kind} onClick={() => setSelected(card.kind)}>Lihat {card.title.toLowerCase()}</button><button onClick={() => downloadCsv(card.kind)}><Download size={15} /> CSV</button></article>)}</section> : null}
    <section className="data-panel ledger-panel">
      <div className="toolbar"><div><strong>{title}</strong><small>{kind === "inventory" ? `Kondisi pada ${formatDate(data.generatedAt)} WIB` : month ? `Periode ${month}` : "Semua periode"}</small></div>{kind === "movements" ? <label className="toolbar-select"><select value={movementType} onChange={(event) => setMovementType(event.target.value)} aria-label="Jenis pergerakan"><option value="">Semua jenis</option><option>Barang masuk</option><option>Penggunaan</option></select></label> : null}</div>
      ...
      {kind === "usages" ? <><div className="usage-summary"><div><span>Penggunaan selesai</span><strong>{formatKg(totals.grams)}</strong><small>{totals.count} sesi selesai</small></div><div><span>Estimasi biaya penggunaan</span><strong>{formatMoney(totals.estimatedCost)}</strong><small>Berdasarkan harga unit dan berat awal penerimaan</small></div></div><Table headers={usageHeaders} rows={period.usages.map((item) => [item.number, item.userName, item.category, sessionStatusLabels[item.status] ?? item.status, formatDate(item.startedAt), item.completedAt ? formatDate(item.completedAt) : "—", item.unitCount, formatNumber(item.totalStartingGrams), item.totalUsedGrams === null ? "—" : formatNumber(item.totalUsedGrams), item.totalReturnedGrams === null ? "—" : formatNumber(item.totalReturnedGrams), item.status === "COMPLETED" ? formatMoney(item.estimatedCost) : "—", item.result ? resultLabels[item.result] : "—", item.notes || "—"])} /><p className="report-note">Sesi selesai masuk periode tanggal selesai; sesi aktif masuk periode tanggal mulai. Biaya yang belum memiliki harga atau berat awal tidak diisi.{totals.incompleteCostCount ? ` ${totals.incompleteCostCount} sesi memiliki data biaya belum lengkap.` : ""}</p></> : null}
