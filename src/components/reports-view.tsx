...
export default function ReportsView({ state, ledgerOnly = false }: { state: ReportState; ledgerOnly?: boolean }) {
  ...
  const usageHeaders = ["Referensi", "Nama pengambil", "Jenis", "Status", "Mulai (WIB)", "Selesai (WIB)", "Unit", "Gram awal", "Gram digunakan", "Gram kembali", "Status gram", "Estimasi biaya (Rp)", "Hasil", "Catatan"];
  ...
  return <>
    ...
    <section className="data-panel ledger-panel">
      ...
      {kind === "usages" ? <>
        ...
        <div className="usage-summary"><div><span>Penggunaan selesai sesuai filter</span><strong>{formatKg(totals.grams)}</strong><small>{totals.count} sesi selesai{totals.pendingMeasurementCount ? ` · ${totals.pendingMeasurementCount} masih sementara` : ""}</small></div><div><span>Estimasi biaya penggunaan</span><strong>{formatMoney(totals.estimatedCost)}</strong><small>Berdasarkan harga unit dan berat awal penerimaan</small></div></div>
        {totals.pendingMeasurementCount ? <div className="warning-strip"><span>Gram dari <strong>{totals.pendingMeasurementCount} sesi</strong> masih berupa cadangan dan akan berubah setelah penimbangan disahkan.</span></div> : null}
        <Table headers={["Detail", "Referensi", "Nama pengambil", "Jenis", "Status", "Mulai (WIB)", "Selesai (WIB)", "Unit", "Gram digunakan", "Hasil"]} rows={usages.map((item) => [
          ...
          item.totalUsedGrams === null ? "—" : `${item.needsWeighing ? "Cadangan " : ""}${formatNumber(item.totalUsedGrams)}`, item.result ? resultLabels[item.result] : "—",
        ])} />
