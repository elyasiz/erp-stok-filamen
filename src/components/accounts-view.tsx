...
export function AccountsView() {
  ...
  const [form, setForm] = useState({ name: "", email: "", role: "COACH" as UserRole, status: "ACTIVE" as AppUser["status"] });
  ...
  const open = (value: AppUser | "new") => { setEditor(value); setForm(value === "new" ? { name: "", email: "", role: "COACH", status: "ACTIVE" } : { name: value.name, email: value.email, role: value.role, status: value.status }); setFormError(""); };
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setNotice(editor === "new" ? `${form.name} sudah terdaftar dan dapat masuk menggunakan email Google tersebut.` : "Perubahan akun tersimpan.");
  ...
  return <div className="accounts-view"><div className="module-heading"><div><span className="eyebrow">AKSES TIM</span><h1>Pengguna & hak akses</h1><p>Daftarkan email Google staf dan tentukan tugasnya.</p></div><button className="button primary" onClick={() => open("new")}><Plus size={17} /> Tambah pengguna</button></div>
