...
function AccountDialog({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  ...
  return <dialog ref={ref} className="account-dialog" onCancel={event => { event.preventDefault(); onClose(); }} aria-labelledby="account-dialog-title"><div className="dialog-head"><h2 id="account-dialog-title">{title}</h2><button autoFocus className="icon-button" onClick={onClose} aria-label="Tutup"><X size={20} /></button></div>{children}</dialog>;
}
...
const actions: Record<string,string> = { OWNER_CREATED: "Mengaktifkan akun Owner", PASSWORD_RESET: "Mereset kata sandi pengguna", PASSWORD_CHANGED: "Mengubah kata sandi pribadi", LOGIN: "Masuk ke aplikasi", USER_CREATED: "Menambahkan pengguna", USER_UPDATED: "Mengubah akun", STOCK_CREATED: "Menambahkan stok", STOCK_UPDATED: "Mengoreksi stok", STOCK_DELETED: "Menghapus stok", RECEIPT_CREATED: "Mencatat penerimaan", RECEIPT_UPDATED: "Mengubah penerimaan", RECEIPT_DELETED: "Menghapus penerimaan", USAGE_STARTED: "Memulai penggunaan", USAGE_COMPLETED: "Menyelesaikan penggunaan", USAGE_COMPLETED_UNWEIGHED: "Mencatat print gagal dengan gram sementara", USAGE_WEIGHING_VERIFIED: "Mengesahkan hasil timbang penggunaan", USAGE_CORRECTION_REQUESTED: "Mengajukan koreksi penggunaan", USAGE_CORRECTION_APPROVED: "Menyetujui koreksi penggunaan", USAGE_CORRECTION_REJECTED: "Menolak koreksi penggunaan" };
