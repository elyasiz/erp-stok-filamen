...
test("all data routes reject anonymous requests before reading or changing stock",async()=> {
  ...
  for(const [file,methods] of [["inventory",["GET","POST"]],["inventory/[id]",["GET","PATCH","DELETE"]],["receipts",["GET","POST"]],["receipts/[id]",["GET","PATCH","DELETE"]],["usages",["GET","POST"]],["usages/[id]",["GET"]],["usages/[id]/complete",["POST"]],["usages/[id]/corrections",["GET","POST"]],["usage-corrections",["GET"]],["usage-corrections/[id]",["PATCH"]],["usage-weighings",["GET"]],["usage-weighings/[id]",["PATCH"]],["reports",["GET"]],["users",["GET","POST"]],["users/[id]",["PATCH"]],["activity",["GET"]],["my-usage",["GET"]]]) {
...
test("coach cannot access administration or costs; forged borrower and cross-origin writes fail",async()=> {
  ...
  for(const file of ["reports","receipts","users","activity","usage-corrections","usage-weighings"]) assert.equal((await load(`src/app/api/v1/${file}/route.ts`).GET(request(`/api/v1/${file}`))).status,403,file);
